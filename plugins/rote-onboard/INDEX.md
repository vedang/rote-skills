...
Four guided slash-command skills that walk a human through getting rote working and keeping it
that way. They're a sequence, not a grab-bag — each ends by naming the next logical step, so
the path becomes muscle memory. This file is the canonical map; each skill links back here.

## The four skills, in the order you meet them
...
| 2 | `/rote-onboard:rote-adapter-create` | Add an adapter for any API — dry-run-first, auth researched from the provider's docs. | adapter-config (tune it) |
...
| 4 | `/rote-onboard:rote-update` | Keep current — update both layers (the `rote` binary **and** these skills), which version independently. | — |
...
- **Discovery:** type `/rote` in the agent — the four `rote-onboard:*` skills appear with their
...
All four skills end a **clean** run on one dry one-liner, keyed to what actually just happened.
...
Rules (identical across all four — keep them consistent):
